...
int32 bmp085GetAltitude(void)  //获得海拔高度，单位厘米
{
    ...
    centimeters =  (int32)(44330 * (1 - 
           pow(((float)pressure / (float)MSLP), 0.1903))) + Altitude_cm_Offset;  
    // converting from float to int32_t truncates toward zero, 
    // 100.999985 becomes 100 resulting in 1 cm error (max).
    return centimeters;
}
